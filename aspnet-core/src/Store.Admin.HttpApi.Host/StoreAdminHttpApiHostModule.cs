...
public class StoreAdminHttpApiHostModule : AbpModule
{
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<StoreDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Store.Domain.Shared"));
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<StoreAdminApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Store.Admin.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(StoreAdminApplicationModule).Assembly);
        });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "Store Admin API", Version = "v1" });
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("Store.Admin");
        if (!hostingEnvironment.IsDevelopment())
        {
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "Store Admin API");
